...
export interface ChannelMeta {
  channel: string
  /** 32-hex channel secret (the PSK); analyzer `secretHex`. */
  channel_hash?: string | null
  country?: string
  region?: string
  /** All countries derived from scopes (a channel can span several). */
  countries?: string[]
  /** Dutch provinces derived from scopes. */
  regions?: string[]
  scopes?: string[]
...
export type SortField = 'alpha' | 'nl_activity' | 'country' | 'region' | 'scope' | 'last_seen' | 'message_amount'
