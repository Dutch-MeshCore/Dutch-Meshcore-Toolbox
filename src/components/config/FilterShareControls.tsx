...
interface Props {
  ...
  onImport: (next: FilterSettings) => void
...
export default function FilterShareControls({ value, onImport, onToast }: Props) {
  const { lang } = useLang()
  const c = copy[lang]
  ...
  async function copyToClipboard() {
    try {
      await navigator.clipboard.writeText(serializeFilterSettings(value))
      toast(c.copied, 'ok')
    } catch {
      toast(c.copyErr, 'err')
    }
  ...
  function download() {
    const blob = new Blob([serializeFilterSettings(value)], { type: 'application/json' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url
    a.download = 'dmc-filter.json'
    a.click()
    URL.revokeObjectURL(url)
  }

  function applyImported(text: string): boolean {
    const parsed = parseSharedFilterSettings(text)
